    ...
    for simnod = 1:size(path_sim,2)
        ...
        for i = 1:nb_of_tries
            ...
            elseif distance_type == 2;          %continuous variable, weighting with inverse square distance
                distance = sqrt((sum(data_event_sim(ind)-data_event_ti(ind)).^2)/nelm);
            end
...
simul=simul*maxti+minti;
disp(['mean minumum distance: ',num2str(mean(bestmin(isnan(ti))))])
